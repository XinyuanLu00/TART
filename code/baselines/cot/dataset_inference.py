...
class TableFormaterInference:
    ...
    def format_data_sample(self, sample):
        ...
        task_str = f"Given the following table and question, generate a step-by-step reasoning explanation and the final answer.\n\n{input_table}"
    ...
    def load_data(self):
        ...
        dataset = []
        for sample in data:
            sample['table_input'] = self.format_data_sample(sample)
    ...
    def inference_on_dataset(self):
        ...
        if self.args.batch_inference:
            for i in tqdm(range(0, len(self.dataset), self.args.batch_size)):
                ...
                input_str = [sample['table_input'] for sample in batch]
                output_str = self.lora_inference.generate_batch(input_str)
                for j, sample in enumerate(batch):
                    sample['output'] = self.post_process_output(output_str[j])
                    output_data.append(sample)
        else:
            # # single inference
            for sample in tqdm(self.dataset):
                input_str = sample['table_input']
                output_str = self.lora_inference.generate([input_str])
                sample['output'] = self.post_process_output(output_str[0])
                output_data.append(sample)
